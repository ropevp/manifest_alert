...
class AlertDisplay(QWidget):
    def __init__(self):
        ...
        # Flag to prevent tree operations from disrupting window state
        self.disable_tree_operations = False
        
        # Flag to track if tree needs refresh when exiting fullscreen/maximized
        ...
        emergency_exit = QShortcut(QKeySequence("Ctrl+Shift+Q"), self)
        emergency_exit.activated.connect(lambda: self.tray_icon.hide() or QApplication.instance().quit())
        ...
        try:
            ...
            # Fallback to basic menu if enhanced menu fails
    ...
    def _toggle_visibility(self):
        ...
        if self.isMinimized():
            # Restore to previous state - don't force maximize
            current_state = self.windowState()
            # Remove minimized flag but keep other states
            ...
            # Not minimized - just bring to front without changing state
    ...
    def update_clock_and_countdown(self):
        ...
        if now.date() != self.current_date:
            ...
            # Only refresh tree if not in fullscreen/maximized
            current_state = self.windowState()
            if not (current_state & Qt.WindowFullScreen or current_state & Qt.WindowMaximized):
                self.populate_list()
            else:
                # Mark for refresh when exiting fullscreen/maximized
        ...
        self.update_visual_alerts()
        
        # If any Active or Missed exists in tree, focus group and maximize
        any_alerting = self.has_active_alerts()
        if any_alerting:
            # DISABLED: Tree operations that constantly disrupt window state
            # The tree expansion logic has been moved to populate_list() only
            # to prevent continuous window state disruption every second
        ...
        try:
            ...
            if self.last_ack_check_time is None or file_mod_time > self.last_ack_check_time:
                ...
                if self.last_ack_check_time is not None:
                    ...
                    # Preserve window state after tree collapse/expand operations
    ...
    def populate_list(self):
        ...
        # SKIP tree operations entirely if in fullscreen/maximized to prevent window movement
        current_state = self.windowState()
        if not (current_state & Qt.WindowFullScreen or current_state & Qt.WindowMaximized):
            for i in range(self.tree_widget.topLevelItemCount()):
                grp = self.tree_widget.topLevelItem(i)
    ...
    def toggle_fullscreen(self):
        ...
        if self.windowState() & Qt.WindowFullScreen:
            ...
            self.full_btn.setIcon(self.style().standardIcon(QStyle.SP_TitleBarMaxButton))
            # Refresh tree when exiting fullscreen (still in maximized, so no refresh yet)
        else:
            # Enter fullscreen
            self.setWindowState(Qt.WindowFullScreen)
    ...
    def collapse_all(self):
        ...
        # Prevent tree operations in fullscreen/maximized to avoid window movement
        current_state = self.windowState()
        if current_state & Qt.WindowFullScreen or current_state & Qt.WindowMaximized:
            return  # Skip operation entirely
            
        # Store state before tree operation
        saved_state = self.windowState()
        saved_pos = self.pos()
        saved_size = self.size()
        
        self.tree_widget.collapseAll()
        
        # FORCE restore exact window state after tree operation
        if saved_state & Qt.WindowFullScreen:
            self.setWindowState(Qt.WindowFullScreen)
        elif saved_state & Qt.WindowMaximized:
            self.setWindowState(Qt.WindowMaximized)
        else:
            self.move(saved_pos)
    ...
    def closeEvent(self, event):
        ...
        # But allow force close with Ctrl+Alt+F4 for emergency exit
        modifiers = QApplication.keyboardModifiers()
        if modifiers == (Qt.ControlModifier | Qt.AltModifier):
            # Emergency exit - actually close
            self.tray_icon.hide()
            QApplication.instance().quit()
            event.accept()
        else:
            # Normal close attempt - just ignore to prevent accidental closure
            event.ignore()
    ...
    def _force_window_state_after_ack(self):
        """FORCE window back to maximized/fullscreen after acknowledgment"""
        # Always force back to maximized or fullscreen after any acknowledgment
        current_state = self.windowState()
        
        if current_state & Qt.WindowFullScreen:
            # Already fullscreen - ensure it stays fullscreen
            self.setWindowState(Qt.WindowFullScreen)
        elif current_state & Qt.WindowMaximized:
            # Already maximized - ensure it stays maximized  
            self.setWindowState(Qt.WindowMaximized)
        else:
            # Normal state - force to maximized for production
            self.setWindowState(Qt.WindowMaximized)
    ...
    def _preserve_window_state_after_ack(self):
        ...
        # If we're in fullscreen or maximized, explicitly restore it
        if current_state & Qt.WindowFullScreen:
            # Force fullscreen state restoration
            self.setWindowState(Qt.WindowFullScreen)
            self.raise_()
            self.activateWindow()
        elif current_state & Qt.WindowMaximized:
            # Force maximized state restoration
            self.setWindowState(Qt.WindowMaximized)
            self.raise_()
            self.activateWindow()
        else:
            # Normal state - ensure visibility and position
            self.show()
            self.raise_()
            self.activateWindow()
            # Restore exact position and size for normal windows
